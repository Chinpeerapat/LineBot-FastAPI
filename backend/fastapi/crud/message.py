...
class MessageService:
    ...
    def get_latest_messages(self, user_id: UUID, n: int) -> List[Message]:
        # Fetch the latest 2n messages (since each pair consists of one user and one server message)
        messages = self.db_sync.query(Message).filter(
